const CreateGroupsAlgorithm = (forms) => {
	...
	*/
	...
	/*
		...
		Create groups of 3 or 4 where possible based on shared choice 1 and 
		compatible times.
		----
		...
		store them in the array, projectAsFirstChoice.

		From there, further filter projectAsFirstChoice into three separate
		arrays based on the students availability (morningCompatibleFilter, 
		eveningCompatibleFilter, and mixedAvailability). Create groups  
		of 3 or 4 for each availabilty type, grabbing from mixedAvailability 
		when possible if an availability type isn't divisible by 3 or 4 yet.
	...
	*/
	modifiedProjectsArray.forEach(project => {
		...
		/* 	If morningCompatibleFilter is already divisible by 3 or 4,
			...
			morningCompatibleFilter has a length of two or more and 
		...
		else if (morningCompatibleFilter.length >= 2 && mixedAvailability.length !== 0) {
			while (true) {
		...
		if (eveningCompatibleFilter.length % 4 === 0) {
		...
		else if (eveningCompatibleFilter.length >= 2 && mixedAvailability.length !== 0) {
			while (true) {
				if (mixedAvailability.length !== 0) {
					// Append a mixed availability student
					let studentToAppend = mixedAvailability[0];
			...
			if (eveningCompatibleFilter.length % 4 === 0) {
				...
				// Remove students from evening array it is divisible again 
				while (true) {
					// 	Pop off the last student 
					eveningCompatibleFilter.pop()
		...
		/* 	If the remaining mixed students length is divisible by 3 or 4, create a group 
			...
			students until it is divisible then create a group. The removed student(s) will 
			have the chance to join another open group meeting their first choice in the next step. */
		if (mixedAvailability.length !== 0) {
	...
	*/
	...
	/* 
		...
		people sharing the same choice_1. If there is such a group, 
		...
		*/
		...
		if (possibleCandidates.length !== 0) {
			for (let i = 0; i < possibleCandidates.length; i++) {
				...
				// For each almost completed group, check each student for a match.
				almostCompletedGroups.forEach(group => {
					group.forEach(student => {
				...
				/* 	If the current iterated candidate doesn't have a choice conflice and their 
	...
	*/
	...
	/* 
		...
		Similar to STEP 6 but this time, allow these groups to grab 
	...
	*/
	...
	/* 	
		...
		compared to the others, thus were never created, or picked projects that already 
		have maxed out groups. 

		We have 2 ways to approach this group: (1) Allow the instructor to look at this group
		and contact the students / manually assign them into a different group, possibly 
		overriding the four student limit or available time constraint at their own discretion. 
		(2) Or trying to auto add the student into any open group that meets their available time 
		and the project is related to one of their disciplines.

		SIDE THOUGHTS - I personally chose approach 1. Though approach 2 would be helpful
		in not needing any communication, it can become unfair to a student if they are placed into
		a random project that meets one of their disciplines at a surface level. For example, the 
		Student Matching System (project 2) and Virtual Reality Game (project 9) both require some
		type of programming, however the two systems being programmed are still very different and the
		student might prefer one type over the other.
	*/
